...
class MonteCarlo():
    ...
    def get_best_move(self):
        ...
        for move in legal_moves:
            ...
            # print("move treated:", move, "state reached", state)
    ...
    def run_simulation(self):
        ...
        while not game.is_over() and move_num < 10000:
            ...
            for move in legal_moves:
                game_copy = game.copy()
                # print('player', game_copy.current_player)
                # print('turns played', game_copy.players[game_copy.current_player].turns_played)
                # print('rolled dice', game_copy.players[game_copy.current_player].rolled_dice)
                # print('move', move, 'game', game_copy.get_state())
                game_copy.do_move(move)
                state = game_copy.get_state()
                moves_states.append((move, state))
            ...
            if all(played.get((player, state)) for move, state in moves_states):
                ...
                # print("UCB", move, state, "\n\n\n")
            else:
                # otherwise, just make an arbitrary decision
                move, state = random.choice(moves_states)
                # print("rand", move, state, "\n\n\n")
...
if __name__ == "__main__":
    ...
    while not g.is_over():
        ...
        g.do_move(move)
        g.end_turn()
        print("Player 1:", g.players[0].hp)
        print("Player 2:", g.players[1].hp)
        print()